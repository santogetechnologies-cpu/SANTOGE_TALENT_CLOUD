  ...
  return (
    <div className="space-y-4">
      <div className="rounded-xl border border-line-soft bg-surface-soft p-4">
        <p className="text-[10px] font-semibold uppercase tracking-widest text-copy-subtle">Concept card</p>
        <p className="mt-1.5 text-sm text-foreground">{concept}</p>
      </div>
      {children(run, lines, () => setLines([]))}
      <div className="flex items-center gap-2">
        {done ? <Chip tone="emerald">Verified · +50 XP awarded</Chip> : <Chip tone="muted">Not yet verified</Chip>}
...
function MernLab() {
  ...
  return (
    <LabFrame id="mern" concept="Express route handlers must return a typed JSON contract. Hit the endpoint with the test runner and inspect the serialized response.">
      {(run, lines, reset) => (
        <>
            ...
            <RunButton
              ...
              onClick={() =>
                run([
                  ...
                  JSON.stringify(
                    { ok: true, learner: { _id: "661f", name: "Ajay Kumar", xp: 1250, tracks: ["mern", "cloud", "aiml"] } },
                    null,
                    2,
                  ),
                  code.includes("res.json") ? "✓ JSON contract assertion passed" : "✗ handler never calls res.json()",
                ])
              }
            />
...
function JavaLab() {
  ...
  return (
    <LabFrame id="java" concept="A Spring Boot controller is only trustworthy with green unit tests. Execute the JUnit 5 suite against the MockMvc context.">
      {(run, lines, reset) => (
        <>
            ...
            <RunButton
              ...
              onClick={() =>
                run([
                  ...
                  code.includes("@GetMapping") ? "  ✓ mapping_annotationPresent()          2ms" : "  ✗ mapping_annotationPresent() FAILED",
...
/* ---------------- 3. AI/ML ---------------- */
const CHUNKS = [
  { id: "chunk_18", text: "Placement gate unlocks after dual 100% verified completion", base: 0.94 },
  { id: "chunk_04", text: "Readiness index weights technical progress at 0.25", base: 0.91 },
  { id: "chunk_27", text: "Twin 30 minute cadence: technical self-study + accelerator", base: 0.87 },
...
function AiLab() {
  ...
  return (
    <LabFrame id="aiml" concept="Retrieval-augmented generation grounds an LLM answer in your own corpus. Query the Pinecone index and inspect cosine similarity per chunk.">
      {(run, lines, reset) => (
        <>
          <TextField label="Similarity query" value={query} onChange={setQuery} />
          <label className="block text-xs font-semibold text-copy-subtle">
            top_k = {k}
            <input type="range" min={1} max={5} value={k} onChange={(e) => setK(Number(e.target.value))} className="mt-2 w-full accent-brand-purple" />
          </label>
          <div className="flex gap-2">
            <RunButton
              ...
              onClick={() =>
                run([
                  ...
                  ...CHUNKS.slice(0, k).map(
                    (c, i) => `  ${i + 1}. ${c.id}  score=${(c.base - i * 0.01).toFixed(3)}  "${c.text}"`,
                  ),
...
function DataLab() {
  ...
  return (
    <LabFrame id="datascience" concept="Before modelling, clean the frame and read the correlation matrix. Nulls silently bias every downstream statistic.">
      {(run, lines, reset) => (
        <>
          ...
          <div className="flex flex-wrap gap-4 text-xs font-semibold text-copy-subtle">
            <label className="flex items-center gap-2">
              <input type="checkbox" checked={dropna} onChange={(e) => setDropna(e.target.checked)} className="accent-brand-emerald" /> df.dropna()
            </label>
            <label className="flex items-center gap-2">
              <input type="checkbox" checked={normalize} onChange={(e) => setNormalize(e.target.checked)} className="accent-brand-emerald" /> min-max normalize
            </label>
          </div>
          <div className="flex gap-2">
            <RunButton
              ...
              onClick={() =>
                run([
                  ...
                  dropna ? ">>> df = df.dropna()   # 1 row removed" : ">>> # nulls retained — corr() will skip pairs",
                  normalize ? ">>> df = (df - df.min()) / (df.max() - df.min())" : "",
                  ">>> df.corr(numeric_only=True)",
                  "              hours   score  attendance",
                  dropna ? "hours         1.000   0.982       0.964" : "hours         1.000   0.947       0.921",
                  dropna ? "score         0.982   1.000       0.971" : "score         0.947   1.000       0.958",
                  dropna ? "attendance    0.964   0.971       1.000" : "attendance    0.921   0.958       1.000",
...
function CloudLab() {
  ...
  return (
    <LabFrame id="cloud" concept="Infrastructure as code makes environments reproducible. Edit the resource block, plan the change, then apply it.">
      {(run, lines, reset) => (
        <>
...
function CyberLab() {
  ...
  return (
    <LabFrame id="cyber" concept="Reconnaissance maps the attack surface. Scan the hardened lab host and confirm which ports the firewall should be filtering.">
      {(run, lines, reset) => (
        <>
          ...
          <div className="flex flex-wrap gap-2">
            {PORTS.map((p) => (
              <button
                key={p.port}
                onClick={() =>
                  setSelected((s) => (s.includes(p.port) ? s.filter((x) => x !== p.port) : [...s, p.port]))
                }
                className={cn(
              ...
              onClick={() =>
                run([
                  ...
                  ...PORTS.filter((p) => selected.includes(p.port)).map(
                    (p) => `${String(p.port + "/tcp").padEnd(9)}${p.state.padEnd(10)}${p.svc.padEnd(9)}${p.banner}`,
                  ),
                  "--- firewall review ---",
                  ...PORTS.filter((p) => selected.includes(p.port)).map((p) => `  ${p.port}: ${p.risk}`),
...
function SreLab() {
  ...
  const rps = 4800;
  return (
    <LabFrame id="sre" concept="Horizontal scaling spreads request load across pods. Watch per-pod RPS and latency change as you scale the deployment.">
      {(run, lines, reset) => (
        <>
          <label className="block text-xs font-semibold text-copy-subtle">
            kubectl scale deploy/itse-api --replicas={replicas}
            <input type="range" min={1} max={12} value={replicas} onChange={(e) => setReplicas(Number(e.target.value))} className="mt-2 w-full accent-brand-cyan" />
          </label>
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
            {Array.from({ length: replicas }, (_, i) => (
              <div key={i} className="rounded-xl border border-line-soft bg-surface-soft p-2.5 font-mono text-[11px]">
            ...
            <RunButton
              ...
              onClick={() =>
                run([
                  ...
                  replicas < 3 ? "⚠ below minimum HA replica count (3)" : "✓ SLO 99.9% availability satisfied",
                ])
              }
            />
            <ResetButton onClick={reset} />
...
function UiuxLab() {
  ...
  return (
    <LabFrame id="uiux" concept="Accessible interfaces need contrast ≥ 7:1 for AAA body text. Test the palette across both viewports before shipping the design token.">
      {(run, lines, reset) => (
        <>
            ...
            {(["mobile", "desktop"] as const).map((v) => (
              <button
                ...
                className={cn(
                  ...
                  viewport === v ? "border-brand-purple/60 bg-brand-purple/10 text-brand-purple" : "border-line-soft bg-surface-soft text-copy-subtle",
                )}
              >
          ...
          <div className="rounded-xl border border-line-soft p-4">
            <div
              className={cn("mx-auto rounded-xl p-5 transition-all", viewport === "mobile" ? "max-w-[280px]" : "max-w-full")}
              style={{ background: bg, color: fg }}
            >
              <p className="font-display text-lg font-bold">Placement Readiness</p>
              <p className="mt-1 text-sm opacity-80">Body copy sample at 14px — check legibility.</p>
            </div>
          </div>
          <p className="font-mono text-xs text-copy-subtle">
            contrast = {ratio.toFixed(2)}:1 ·{" "}
            <span className={ratio >= 7 ? "text-brand-emerald" : ratio >= 4.5 ? "text-brand-amber" : "text-brand-rose"}>
            ...
            <RunButton
              ...
              onClick={() =>
                run([
                  ...
                  ratio >= 7 ? "✓ token approved for design system" : "⚠ adjust palette before promoting the token",
                ])
              }
            />
            <ResetButton onClick={reset} />
...
function QaLab() {
  ...
  return (
    <LabFrame id="qa" concept="Regression suites are your safety net. Run the Cypress specs and read the step-by-step log; inject a failure to see the diagnostic output.">
      {(run, lines, reset) => (
        <>
          <label className="block text-xs font-semibold text-copy-subtle">
              ...
              onClick={() =>
                run([
                  ...
                  ...SPECS.flatMap((s, i) => [
                    ...
                    i === failIndex ? `      AssertionError: expected 'locked' to equal 'unlocked'\n      at gateway.spec:42` : "",
                  ]).filter(Boolean),
                  `Tests: ${SPECS.length} · Passing: ${failIndex === -1 ? SPECS.length : SPECS.length - 1} · Failing: ${failIndex === -1 ? 0 : 1}`,
                  failIndex === -1 ? "✓ Suite green — safe to merge" : "✗ Pipeline blocked — fix the failing spec",
                ])
              }
            />
            <ResetButton onClick={reset} />
...
function MobileLab() {
  ...
  return (
    <LabFrame id="mobile" concept="Hot reload keeps native iteration fast. Trigger a rebuild, then fire a push notification into the device mock.">
      {(run, lines, reset) => (
        <>
          <div className="flex flex-wrap items-start gap-5">
            <div className="mx-auto w-[220px] shrink-0 rounded-[28px] border-4 border-line-soft bg-surface-dark p-3">
              ...
              <p className="mt-1 font-mono text-[10px] text-copy-subtle">build #{build} · hot reload</p>
              <div className="mt-3 space-y-1.5">
                {["Dashboard", "Labs", "Accelerator"].map((s) => (
                  <div key={s} className="rounded-lg bg-surface-soft px-2 py-1.5 text-[11px] text-foreground">{s}</div>
                ))}
              </div>
            </div>
          ...
          <div className="flex gap-2">
            ...
            />
            <ResetButton onClick={() => { setPushed(false); reset(); }} />
          </div>
          <Console lines={lines} />
        </>
      )}
...
function MedicalLab() {
  ...
  return (
    <LabFrame id="medical" concept="Clean claim rate depends on valid ICD-10 diagnosis and CPT procedure pairing. Validate the codes, then project reimbursement quality.">
      {(run, lines, reset) => (
        <>
          <div className="grid gap-3 sm:grid-cols-2">
          ...
          <label className="block text-xs font-semibold text-copy-subtle">
            ...
            <input type="range" min={20} max={400} step={10} value={claims} onChange={(e) => setClaims(Number(e.target.value))} className="mt-2 w-full accent-brand-rose" />
          </label>
          <div className="flex gap-2">
            <RunButton
              ...
              onClick={() => {
                ...
                run([
                  ...
                  rate > 95 ? "✓ Claim batch ready for submission" : "⚠ Route to coder review queue",
                ]);
              }}
            />
            <ResetButton onClick={reset} />
...
function MarketingLab() {
  ...
  return (
    <LabFrame id="marketing" concept="Budget reallocation is the fastest ROAS lever. Shift spend between channels and watch blended return and CAC respond.">
      {(run, lines, reset) => (
        <>
          <label className="block text-xs font-semibold text-copy-subtle">
            Monthly budget ₹{budget.toLocaleString("en-IN")}
            <input type="range" min={50000} max={1000000} step={10000} value={budget} onChange={(e) => setBudget(Number(e.target.value))} className="mt-2 w-full accent-brand-amber" />
          </label>
          <label className="block text-xs font-semibold text-copy-subtle">
            Paid search {split}% · paid social {100 - split}%
            <input type="range" min={0} max={100} value={split} onChange={(e) => setSplit(Number(e.target.value))} className="mt-2 w-full accent-brand-amber" />
          </label>
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
            {[
            ...
            <RunButton
              ...
              onClick={() =>
                run([
                  ...
                  `projected revenue ₹${Math.round(revenue).toLocaleString("en-IN")}`,
                  split > 70 ? "⚠ heavy search concentration — audit frequency capping" : "✓ channel mix within variance guardrails",
                ])
              }
            />
            <ResetButton onClick={reset} />
...
function SapLab() {
  ...
  return (
    <LabFrame id="sap" concept="Every FI document must balance debit and credit before posting. Fill the GL line items and post through transaction FB50.">
      {(run, lines, reset) => (
        <>
          <div className="grid gap-3 sm:grid-cols-2">
          ...
          <label className="block text-xs font-semibold text-copy-subtle">
            ...
            <input type="range" min={1000} max={500000} step={1000} value={amount} onChange={(e) => setAmount(Number(e.target.value))} className="mt-2 w-full accent-brand-blue" />
          </label>
          <div className="flex gap-2">
            <RunButton
              label="Post (FB50)"
...
function HrLab() {
  ...
  return (
    <LabFrame id="hr" concept="A compliant salary structure balances tax efficiency with statutory floors. Build the CTC breakup and read the monthly take-home.">
      {(run, lines, reset) => (
        <>
          <label className="block text-xs font-semibold text-copy-subtle">
            Annual CTC ₹{ctc.toLocaleString("en-IN")}
            <input type="range" min={240000} max={2400000} step={20000} value={ctc} onChange={(e) => setCtc(Number(e.target.value))} className="mt-2 w-full accent-brand-emerald" />
          </label>
          <label className="flex items-center gap-2 text-xs font-semibold text-copy-subtle">
            <input type="checkbox" checked={metro} onChange={(e) => setMetro(e.target.checked)} className="accent-brand-emerald" />
          ...
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
            {[
              ...
              <div key={l as string} className="rounded-xl border border-line-soft bg-surface-soft p-2.5">
                <p className="text-[10px] uppercase tracking-widest text-copy-subtle">{l}</p>
                <p className="mt-1 font-mono text-sm text-foreground">₹{(v as number).toLocaleString("en-IN")}</p>
              </div>
            ))}
          </div>
          <div className="flex gap-2">
...
function BiLab() {
  ...
  return (
    <LabFrame id="bianalytics" concept="DAX measures turn raw rows into KPIs. Choose a measure, evaluate the formula, and read the refreshed revenue visual.">
      {(run, lines, reset) => (
        <>
            ...
            {(["ytd", "growth", "variance"] as const).map((m) => (
              <button
                ...
                className={cn(
                  ...
                  measure === m ? "border-brand-purple/60 bg-brand-purple/10 text-brand-purple" : "border-line-soft bg-surface-soft text-copy-subtle",
                )}
              >
              ...
              onClick={() =>
                run([
                  ...
                  measure === "ytd"
                    ...
                    : measure === "growth"
                      ? MONTHS.slice(1)
                          .map((x, i) => `${x.m}: ${(((x.rev - MONTHS[i]!.rev) / MONTHS[i]!.rev) * 100).toFixed(1)}%`)
                          .join("  ·  ")
                      : MONTHS.map((x) => `${x.m}: ${x.rev >= x.target ? "+" : ""}₹${(x.rev - x.target).toLocaleString("en-IN")}`).join("  ·  "),
